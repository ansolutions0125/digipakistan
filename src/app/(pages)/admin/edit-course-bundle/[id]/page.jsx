...
const EditCourseBundle = ({ params }) => {
  const unwrappedParams = use(params); // Unwraps the params Promise

  const courseBundleId = unwrappedParams.id;
  ...
  useEffect(() => {
    const fetchCourseBundleData = async () => {
      const courseData = await getSingleCourseBundle(courseBundleId);
      const allcourses = await getAllCourses();
      setBundleCourse(courseData.data);
      setAvailableCourses(allcourses.data);
      setFilteredCourses(allcourses.data);
      setFormData((prev) => ({ ...prev, ...courseData.data }));
    };
    fetchCourseBundleData();
  }, [courseBundleId]); // Only run on initial load and when courseBundleId changes
  ...
  const handleSubmit = async () => {
    if (validateStep()) {
      ...
      await updateDoc(doc(firestore, "courses_bundle", courseBundleId), {
        ...formData,
  ...
  const handleRemove = (courseTitle) => {
    const updatedSelectedCourses = bundleCourse.selectedCourses.filter(
      (course) => course.courseTitle !== courseTitle
    );
    const removedCourse = bundleCourse.selectedCourses.find(
      (course) => course.courseTitle === courseTitle
    );
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={formData.courseBundlePrice}
                        onChange={handleChange}
                      />
                    </div>
                    <div>
