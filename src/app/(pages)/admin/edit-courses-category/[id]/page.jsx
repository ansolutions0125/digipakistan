...
const EditCourseBundle = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      const categoryRef = doc(firestore, "courses_category", id);
      ...
      if (categoryDoc.exists()) {
        ...
        setFormData({
          ...
          coursesInThisCategory: data.coursesInThisCategory || [],
        });
        setcoursesInThisCategory(data.coursesInThisCategory || []);
        setAvailableCourses((prevCourses) =>
          prevCourses.filter(
            (course) =>
              !data.coursesInThisCategory.some(
                (selected) => selected.courseTitle === course.courseTitle
              )
          )
        );
      } else {
        showToast("Category not found.", "error", 2000);
      }
    };
  ...
  const handleUpdate = async () => {
    if (validateForm()) {
      ...
      showToast("Category has been updated successfully", "success", 2000);
    }
  };
