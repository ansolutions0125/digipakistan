...
const GenerateChallan = () => {
  ...
  const ChallanGenerator = (challanData) => {
    ...
    const drawSection = (title, offsetX) => {
      ...
      doc.setFontSize(20);
      doc.text("DigiPAKISTAN", offsetX + sectionWidth / 2, 15, { align: "center" });
      doc.setFontSize(16);
      doc.text("Course Fee Challan", offsetX + sectionWidth / 2, 25, { align: "center" });
      ...
      if (offsetX + sectionWidth < pageWidth) {
        ...
        doc.line(offsetX + sectionWidth, 30, offsetX + sectionWidth, pageHeight - 20); // Dotted line
      }
    };
  ...
  const handlePaymentRequest = async () => {
    try {
      ...
      const response = await fetch("/api/pms-payment-info", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          websiteId: "your-website-id",
          requestedPaymentGateway: "PayFast",
          user: userData,
          course: {
            courseTitle: "",
            price: "",
  ...
  return (
    <div className="flex items-center justify-center p-3 lg:p-5">
      <div className="w-full min-h-screen rounded-xl bg-white flex flex-col text-center p-5 ">
        <h1 className="text-2xl lg:text-5xl font-bold rounded lg:p-3  ">
        ...
        <div className="flex flex-col ">
          ...
          <p className="">Course Fee: 0</p>
        </div>
        <div className="flex flex-col mt-6">
          <h1 className="font-bold lg:text-2xl">For Overseas Applicants</h1>
          <p className="">Registration: --$</p>
          <p className="">Course Fee: 0</p>
        </div>
        ...
        <div className="flex gap-3 mt-4 flex-col lg:flex-row items-center justify-center">
          <button
            ...
            onClick={() => (
              handleGatewaySelection("PayFast"), handlePaymentRequest()
            )}
          ...
          >
            {gatewayLoading && selectedGateway === "PayFast" ? (
              <div role="status">
                <svg
                  aria-hidden="true"
          ...
          <button
            onClick={() => (
              handleChallanGenerate())}
            className="bg-primary hover:bg-second duration-300  text-white p-3 rounded"
          >
